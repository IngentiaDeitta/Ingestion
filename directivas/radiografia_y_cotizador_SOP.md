...
- **Acceso a Cotizaciones Existentes:** Las cotizaciones y propuestas registradas se acceden a través de `/propuestas/:id` ([ProposalDetail.tsx](file:///c:/Ingestion/src/pages/ProposalDetail.tsx)). No redirigir propuestas existentes a `/smart-quoter`, ya que esa ruta se utiliza exclusivamente para el asistente de creación.
- **Edición y Trazabilidad de Envíos:** Tanto en [`ProposalDetail.tsx`](file:///c:/Ingestion/src/pages/ProposalDetail.tsx) como en el listado [`Propuestas.tsx`](file:///c:/Ingestion/src/pages/Propuestas.tsx) se permite la edición del estado comercial, monto total (USD), fecha real de envío (`sent_date`) y todos los campos descriptivos de la propuesta.
